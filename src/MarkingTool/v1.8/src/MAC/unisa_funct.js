       ...
       if(hasRubricAttached) {
           ...
           if(choice == 4) {
              ...
              if(!skipAll) {
                ...
                var newRubricPath = "/Applications/UNISA/Rubrics/" + fileName + "_MARK" + attachedRubricMark + "_RUBRIC.pdf";
...
var addBlankPage = app.trustedFunction(
  function(aNewDoc) {
     ...
     aNewDoc.insertPages({
       ...
       cPath: "/Applications/UNISA/BlankSheet.pdf",
       nStart: 0
     });
...
var readCommentTextFile = app.trustedFunction(
  function(aNewDoc, type) {
      ...
      var filepath = "";
      if(type == "COMM_ENGINE") {
         filepath = "/Applications/UNISA/comm_engine.txt";
      } else
      if(type == "TOT_ENGINE") {
         filepath = "/Applications/UNISA/tot_engine.txt";
      } else
      if(type == "RUBRIC_ENGINE") {
         filepath = "/Applications/UNISA/rubric_engine.txt";
      }
...
var coordinateAnnotInSpace = app.trustedFunction(
   function(aNewDoc, annot, count) {
      var pageSizeBox = aNewDoc.getPageBox("Crop");
      var width   = pageSizeBox[2] - pageSizeBox[0];
      var height  = pageSizeBox[1] - pageSizeBox[3];
...
var addCommentPage = app.trustedFunction(
  function(aNewDoc) {
      var aRect = [0,612,792,0];
      ...
      var edtHeader = aNewDoc.addField("edtCommentHeader", "text", newPageNumber, [lx, ly, rx, ry]);
...
var addCommentBlock = app.trustedFunction(
  function(aNewDoc, uniqueNumber, commentNumber, commentPageNumber, content, rectComment, rectLabel) {
      rectLabel[1]    -= 1;
      rectComment[1]  -= 1;
  
      // CREATE COMMENT
      var edtCommentDesc = aNewDoc.addField("EDTCOMMENTDESCRIPTION:" + uniqueNumber, "text", commentPageNumber, rectComment);                              
      ...
      var edtCommentNumber = aNewDoc.addField("EDITCOMM:" + uniqueNumber, "text", commentPageNumber, rectLabel);
  ...
  function(aNewDoc) {
     ...
     if(rubricDoc == null) {
       ...
       if(rubricFile != "</empty>") {
           ...
           currentRubricOpened = "/Applications/UNISA/Rubrics/" + fileName + "_UNMARKED_RUBRIC.pdf";
           rubricDoc.saveAs({
